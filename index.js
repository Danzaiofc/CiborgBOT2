...
conn.sendMessage(id, 'Owner CIBORG wa.me/+6283804910151' ,MessageType.text);
}
else if (text == 'affis'){
conn.sendMessage(id, 'Aku BOT nya CIBORG' ,MessageType.text);
}
else if (text == 'audio'){
  ...
  var teks = text.replace(/#nulis /, '')
    ...
    .then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, '[SABAR] lagi di carik ya asu...', MessageType.text)
  ...
  var teks = text.replace(/#texthunder /, '')
    ...
    .then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, ' BENTAR BG LAGI DI CARIK', MessageType.text)
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, buf, MessageType.image)
        })
    })
    ...
    axios.get(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=${bg}`).then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, '[SABAR] lagi di carik ya asu...', MessageType.text, { quoted: m })
  ...
  var teks = text.replace(/%nulis2 /, '')
    axios.get(`https://mhankbarbars.herokuapp.com/nulis?text=${teks}&apiKey=N2Ws9kp3KTDYtry5Jjyz`).then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, 'Bot Lagi Nulis ya bg 📝', MessageType.text)
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, buf, MessageType.image)
        })
    })
  ...
  var teks = text.replace(/#cooltext /, '')
    ...
    .then((res) => {
      imageToBase64(res.data.image)
        .then(
          (ress) => {
            conn.sendMessage(id, ' SEDANG DIPROSES COK SABAR', MessageType.text)
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, buf, MessageType.image)
        })
    })
...
axios.get(`https://arugaz.herokuapp.com/api/dewabatch?q=${teks}`).then((res) => {
	conn.sendMessage(id, '[BENTAR] sabar lagi di proses woii...', MessageType.text)
...
axios.get(`https://arugaz.herokuapp.com/api/indohot`).then((res) => {
	conn.sendMessage(id, '[TOBAT] woi tobat goblok...', MessageType.text)
...
if (text.includes("#sholat")){
  ...
  conn.sendMessage(id, '[ASIK] ada yg tobat...', MessageType.text)
...
axios.get('https://api.banghasan.com/quran/format/json/acak').then((res) => {
    ...
    const hs = res.data.acak.id.ayat;
    const ket = `${hs}`.replace(sr, '');
    let hasil = `[${cket}]   ${res.data.acak.ar.teks}\n\n${res.data.acak.id.teks}(QS.${res.data.surat.nama}, Ayat ${ket})`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
conn.sendMessage(id, menu.menu(id,  XIBORG, corohelp, tampilTanggal, tampilWaktu, instagram, nomer, aktif, groupwa, youtube) ,MessageType.text);
}
if (text == '#donasi'){
const corohelp = await get.get('https://covid19.mathdro.id/api/countries/id').json()
var date = new Date();
  ...
  var teks = text.replace(/#text3d /, '')
    ...
    .then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, ' SEDANG DIPROSES BENTAR BG', MessageType.text)
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, buf, MessageType.image)
        })
    })
...
if (text.includes("#ytmp4"))
   {
      ...
      {
         conn.sendMessage(id, "maaf, link yang anda kirim gk bisa kwkwkwk", MessageType.text)
      }
      ytdl.getInfo(videoid[1]).then(info =>
      {
         if (info.length_seconds > 1000)
         {
            conn.sendMessage(id, " videonya kepanjangan pendekin sikit", MessageType.text)
         }
         else
         {
  ...
  var teks = text.replace(/#ssweb /, '')
    ...
    .then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, ' SEDANG DIPROSES SILAHKAN TIDUR', MessageType.text)
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, buf, MessageType.image)
        })
    })
